...
### 기술적 제약
- **Keep-alive 요구사항**: Chrome MV3는 최소 1분의 keep-alive 간격이 필요합니다. 긴 처리 작업은 몇 분이 걸릴 수 있습니다. 확장 프로그램은 초공격적인 keep-alive (10초마다 ping)를 사용하여 service worker가 중지되는 것을 방지합니다.
...
| `alarms` | 긴 작업(큰 기사, 번역) 중에 백그라운드 service worker를 활성 상태로 유지합니다. Chrome Manifest V3는 30초 후 service worker를 일시 중지하지만 기사 처리에는 몇 분이 걸릴 수 있습니다. 간격은 MV3 규칙에 따라 ≥1분으로 설정됩니다. |
